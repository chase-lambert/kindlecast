...
pub fn send_epub(config: &Config, title: &str, source: &str, epub_path: &Path) -> Result<()> {
    ...
    let email = Message::builder()
        ...
        .to(config.device_email.parse::<Mailbox>()?)
        .subject(title)
        .multipart(
            MultiPart::mixed()
                .singlepart(SinglePart::plain(format!(
                    "Generated by RustyPub from {source}."
                )))
    ...
    mailer
        .send(&email)
        .context("SMTP send failed; check the network, app password, and SMTP settings")?;
    Ok(())
}
...
mod tests {
    ...
    fn oversized_epub_is_rejected_before_message_or_smtp_configuration() {
        ...
        let deliberately_invalid = Config {
            device_email: "not a mailbox".to_string(),
